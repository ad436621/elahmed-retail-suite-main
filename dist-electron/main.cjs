...
function initializeDatabase() {
  ...
  const dbPath = path$1.join(userDataPath, isDev ? "retail_dev.sqlite" : "retail_prod.sqlite");
...
function createWindow() {
  mainWindow = new BrowserWindow({
    width: 1280,
    height: 800,
    icon: path$1.join(__dirname$1, "../public/logo.png"),
    webPreferences: {
...
ipcMain.handle("save-image", async (event, base64Data) => {
  try {
    const userDataPath = app.getPath("userData");
    const imagesDir = path$1.join(userDataPath, "images");
    if (!require$$0.existsSync(imagesDir)) {
      require$$0.mkdirSync(imagesDir, { recursive: true });
    }
    ...
    const filePath = path$1.join(imagesDir, fileName);
    require$$0.writeFileSync(filePath, buffer);
