...
import type { MobileAccessory, MobileSparePart } from '@/domain/types';

type MobileSubInventoryRecord = MobileAccessory | MobileSparePart;
type MobileSubSection = 'accessories' | 'spare-parts';

interface MobileSubInventoryViewItem extends ProductDetailsData {
  _raw: MobileSubInventoryRecord;
  subcategory?: string;
  costPrice: number;
}

interface MobileSubInventoryForm {
  name: string;
  barcode: string;
  category: string;
...
const emptyForm = (): MobileSubInventoryForm => ({
  name: '',
  barcode: '',
  category: '',
  condition: 'new',
...
export default function MobileSubInventoryPage<T extends MobileSubInventoryRecord>({ config }: { config: MobileSubInventoryConfig<T> }) {
  const navigate = useNavigate();
  const { toast } = useToast();
  const { confirm } = useConfirm();

  const items = useInventoryData(config.getItems, [config.inventoryStorageKey]);
  const mobileCompanies = useInventoryData(() => getMobiles(), ['gx_mobiles_v2']);
  const [categories, setCategories] = useState<string[]>(() => loadCats(config.categoryStorageKey, config.defaultCategories));
  ...
  const [form, setForm] = useState<MobileSubInventoryForm>(emptyForm());
  const [selectedDetails, setSelectedDetails] = useState<MobileSubInventoryViewItem | null>(null);

  const debouncedSearch = useDebounce(search, 250);
  const debouncedModel = useDebounce(modelFilter, 250);

  const unifiedItems = useMemo<MobileSubInventoryViewItem[]>(
    () =>
      items.map((item) => {
        ...
        return {
          ...
          profitMargin: typeof item.profitMargin === 'number' ? item.profitMargin : calculateProfitAmount(costPrice, item.salePrice),
  ...
  const uniqueCategories = useMemo(() => {
    const values = Array.from(new Set([...categories, ...items.map((item) => item.category).filter(Boolean)])).filter(Boolean) as string[];
    return values.sort();
  }, [categories, items]);

  const uniqueBrands = useMemo(() => {
    const values = new Set<string>();
    mobileCompanies.forEach((item) => {
      ...
      return [
        {
          ...
          label: 'اسم المنتج',
          type: 'text',
          placeholder: 'ابحث باسم الاكسسوار',
          value: search,
          onChange: setSearch,
        },
        {
        ...
        },
        {
          ...
          label: 'الموديل',
          type: 'text',
          placeholder: 'موديل الهاتف أو الجهاز',
          value: modelFilter,
  ...
  const openEdit = (item: MobileSubInventoryViewItem) => {
    setForm({
      name: item.name,
      barcode: item.barcode || '',
      category: item.category || '',
      ...
      boxNumber: item._raw.boxNumber || '',
      taxExcluded: item._raw.taxExcluded || false,
      notes: item.notes || '',
      description: item.description || '',
      image: item.image,
    });
  ...
  const openDetails = (item: MobileSubInventoryViewItem) => setSelectedDetails(item);
  ...
  const handleDelete = async (item: MobileSubInventoryViewItem) => {
    const ok = await confirm({
      title: 'حذف منتج',
      message: `سيتم حذف "${item.name}" من ${config.pageTitle}. هل تريد المتابعة؟`,
      confirmLabel: 'حذف',
      danger: true,
    });
    if (!ok) return;
  ...
  return (
    <div className="space-y-5 animate-fade-in" dir="rtl">
      <div className="flex gap-2 flex-wrap">
        ...
        >
          <Smartphone className="h-4 w-4" /> الموبايل
        </button>
        <button
          ...
          className={cn(
            ...
            config.section === 'accessories' ? 'bg-cyan-600 text-white ring-2 ring-cyan-300 ring-offset-1' : 'bg-muted text-muted-foreground hover:bg-cyan-600 hover:text-white',
          )}
        >
          <Headphones className="h-4 w-4" /> اكسسوارات الموبايل
        </button>
        <button
          onClick={() => navigate('/mobiles/spare-parts')}
          className={cn(
            'flex items-center gap-2 rounded-xl px-5 py-2.5 text-sm font-bold shadow-md transition-all',
            config.section === 'spare-parts' ? 'bg-orange-600 text-white ring-2 ring-orange-300 ring-offset-1' : 'bg-muted text-muted-foreground hover:bg-orange-600 hover:text-white',
          )}
        >
          <Wrench className="h-4 w-4" /> قطع غيار الموبايل
        </button>
      </div>
        ...
        <div className="flex items-center gap-3">
          <div
            className={cn(
              ...
              config.section === 'accessories'
                ? 'border-cyan-200 bg-cyan-100 dark:border-cyan-500/20 dark:bg-cyan-500/15'
                : 'border-orange-200 bg-orange-100 dark:border-orange-500/20 dark:bg-orange-500/15',
            )}
          >
            {config.section === 'accessories' ? <Headphones className="h-5 w-5 text-cyan-600 dark:text-cyan-400" /> : <Wrench className="h-5 w-5 text-orange-600 dark:text-orange-400" />}
          </div>
          <div>
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-3 py-2">
                          ...
                          <div className="text-[11px] text-muted-foreground">{[item.subcategory, item.model, item.color].filter(Boolean).join(' • ') || '—'}</div>
                        </td>
                        ...
                        <td className="px-3 py-2 text-center">
                          <span className={cn('inline-flex items-center gap-1 rounded-full px-2.5 py-0.5 text-xs font-bold', isLowStock ? 'bg-red-100 text-red-600 dark:bg-red-500/15 dark:text-red-400' : 'bg-muted text-foreground')}>
                            {item.quantity}
                            {isLowStock && <Info className="h-3 w-3" />}
                          </span>
                        </td>
            ...
            <div className="my-8 w-full max-w-2xl rounded-2xl border border-border bg-card shadow-2xl animate-scale-in" onClick={(event) => event.stopPropagation()}>
              ...
              <div className="space-y-4 p-4">
                ...
                <div className="grid grid-cols-2 gap-3 rounded-xl border border-border/40 bg-muted/20 p-3 sm:grid-cols-3">
                  <div className="col-span-full mb-1 text-xs font-bold text-primary">{config.formSpecsTitle}</div>
                  <div>
                    <label className="mb-1 block text-xs text-muted-foreground">{config.section === 'accessories' ? 'نوع الاكسسوار' : 'نوع القطعة'}</label>
                    <input value={form.subcategory} onChange={(event) => setForm((current) => ({ ...current, subcategory: event.target.value }))} className={IC} placeholder={config.subcategoryPlaceholder} />
                  </div>
                  <div>
                    <label className="mb-1 block text-xs text-muted-foreground">{config.section === 'accessories' ? 'الموديل' : 'موديل الهاتف'}</label>
                    <input value={form.model} onChange={(event) => setForm((current) => ({ ...current, model: event.target.value }))} className={IC} placeholder={config.modelPlaceholder} />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <input value={form.brand} onChange={(event) => setForm((current) => ({ ...current, brand: event.target.value }))} className={IC} list="mobile-sub-brands" placeholder="اكتب أو اختر" />
                    <datalist id="mobile-sub-brands">
                  ...
                  <div>
                    ...
                    <input value={form.supplier} onChange={(event) => setForm((current) => ({ ...current, supplier: event.target.value }))} className={IC} list="mobile-sub-suppliers" placeholder="اكتب أو اختر" />
                    <datalist id="mobile-sub-suppliers">
                  ...
                  <div>
                    ...
                    <input value={form.source} onChange={(event) => setForm((current) => ({ ...current, source: event.target.value }))} className={IC} list="mobile-sub-sources" placeholder="اكتب أو اختر" />
                    <datalist id="mobile-sub-sources">
                  ...
                  <div>
                    <label className="mb-1 block text-xs text-muted-foreground">رقم الكرتونة</label>
                    <input value={form.boxNumber} onChange={(event) => setForm((current) => ({ ...current, boxNumber: event.target.value }))} className={IC} />
                  </div>
                  <div className="flex items-end">
                    <label className="flex items-center gap-2 text-xs font-semibold">
                      <input type="checkbox" checked={form.taxExcluded} onChange={(event) => setForm((current) => ({ ...current, taxExcluded: event.target.checked }))} className="h-4 w-4 rounded border-border text-primary focus:ring-primary" />
                      الضريبة غير شاملة
                    </label>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="mb-1 block text-xs font-bold text-amber-600 uppercase">حد التنبيه</label>
      ...
      <ExcelColumnMappingDialog
        ...
        onDataSave={(rows) => {
          rows.forEach((row) => {
            config.addItem(config.buildExcelItem(row));
          });
        }}
      />
    </div>
