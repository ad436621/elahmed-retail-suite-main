...
const emptyForm = {
    ...
    model: '', color: '', processor: '', notes: '', description: '', image: undefined as string | undefined
};
...
export default function ComputersInventory() {
    ...
    const fetchCategories = () => getCategoriesBySection('computer');
    ...
    const unifiedProducts = useMemo(() => {
        ...
        computers.forEach(c => {
            ...
            list.push({
                _raw: c, _type: 'device',
                id: c.id, name: c.name, barcode: c.barcode, image: c.image, description: c.description,
                quantity: c.quantity, salePrice: c.salePrice, newCostPrice: c.newCostPrice, oldCostPrice: c.oldCostPrice,
                category: c.category, condition: c.condition || 'new', categoryName: cat?.name,
                model: c.model, processor: c.processor, color: c.color
            })
        });
        ...
        return list;
    }, [computers, accessories, categories]);
    ...
    const filteredList = useMemo(() => {
        ...
        if (search) {
            ...
            res = res.filter(p =>
                ...
                (p.processor && p.processor.toLowerCase().includes(sl))
            );
        }
        return res;
    ...
    const handleCategorySubmit = () => {
        ...
        addCategory({ name: newCatName, section: 'computer', type: newCatType });
        setCategories(fetchCategories());
    ...
    const handleFormSubmit = () => {
        ...
        if (selectedCat.type === 'device') {
            const payload: any = {
                ...
                deviceType: 'laptop', quantity: f.quantity, model: f.model, processor: f.processor, color: f.color,
            ...
            if (editId) updateComputer(editId, payload);
            else addComputer(payload);
        } else {
            const payload: any = {
            ...
            if (editId) updateComputerAccessory(editId, payload);
            else addComputerAccessory(payload);
        }
    ...
    const openEdit = (item: any) => {
        setF({
            ...
            model: item.model || '', processor: item.processor || '', color: item.color || '',
            ...
            <div className="flex gap-2 w-full overflow-x-auto hide-scrollbar pb-2 pt-1 px-1 -mx-1">
                ...
                {categories.map(c => (
                    <button key={c.id} onClick={() => setActiveFilter(c.id)}
                        className={`shrink-0 rounded-xl px-4 py-2 text-sm font-semibold border transition-all flex items-center gap-2 ${activeFilter === c.id ? 'bg-indigo-50 text-indigo-700 border-indigo-300 shadow-sm' : 'bg-card border-border text-muted-foreground hover:text-foreground hover:bg-muted/50'}`}>
                        {c.type === 'device' ? <Laptop className="h-4 w-4 opacity-70" /> : <Headphones className="h-4 w-4 opacity-70" />}
                        {c.name}
                    </button>
                ))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {filteredList.length === 0 ? (
                        <div className="col-span-4 py-20 text-center text-muted-foreground bg-card rounded-2xl border border-dashed border-border">
                            <Laptop className="h-14 w-14 mx-auto mb-4 opacity-15" />
                            ...
                            <tbody>
                                {filteredList.length === 0 ? (
                                    ...
                                    const details = item._type === 'device'
                                        ? [item.model, item.processor, item.color].filter(Boolean).join(' · ')
                                        ...
                                        <tr key={item.id} className={`border-b border-border/40 hover:bg-muted/20 transition-colors ${i % 2 !== 0 ? 'bg-muted/10' : ''}`}>
                                            ...
                                            <td className="px-3 py-2 text-left">
                                                <div className="flex justify-end gap-1.5">
                                                    ...
                                                    <button onClick={() => { item._type === 'device' ? deleteComputer(item.id) : deleteComputerAccessory(item.id); setTimeout(() => window.dispatchEvent(new Event('local-storage-sync')), 100); }} className="rounded-lg p-1.5 hover:bg-red-50 text-destructive transition-colors"><Trash2 className="h-3.5 w-3.5" /> </button>
                                                </div>
                                            </td>
                                        </tr>
                                    )
                    ...
                    <div className="w-full max-w-xl rounded-2xl border border-border bg-card shadow-2xl animate-scale-in my-8">
                        ...
                        <div className="p-4 space-y-4">
                            ...
                            {activeCategoryType === 'device' ? (
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 bg-muted/20 p-3 rounded-xl border border-border/40">
                                    <div className="col-span-full mb-1"><span className="text-xs font-bold text-primary flex items-center gap-1"><Laptop className="w-3.5 h-3.5" /> مواصفات الجهاز</span></div>
                                    ...
                                    <div><label className="mb-1 block text-xs text-muted-foreground">المعالج (Processor)</label><input value={f.processor} onChange={e => setF(p => ({ ...p, processor: e.target.value }))} className={IC} /></div>
                    ...
                    <div className="w-full max-w-sm rounded-2xl bg-card border border-border shadow-2xl p-5 animate-scale-in">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <input value={newCatName} onChange={e => setNewCatName(e.target.value)} placeholder="مثال: لابتوبات قيمنق" className={IC} autoFocus />
                            </div>
                            <div>
