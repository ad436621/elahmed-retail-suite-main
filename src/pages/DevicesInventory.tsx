...
const emptyForm = {
    ...
    model: '', color: '', notes: '', description: '', image: undefined as string | undefined
};
...
function ProductCard({ item, onEdit, onDelete }: { item: any; onEdit: () => void; onDelete: () => void; }) {
    ...
    const extras = [item.model, item.color].filter(Boolean).join(' · ');
...
export default function DevicesInventory() {
    ...
    const fetchCategories = () => getCategoriesBySection('device');
    ...
    const unifiedProducts = useMemo(() => {
        ...
        return list;
    }, [devices, accessories, categories]);
    ...
    const handleCategorySubmit = () => {
        ...
        addCategory({ name: newCatName, section: 'device', type: newCatType });
        setCategories(fetchCategories());
    ...
    const handleFormSubmit = () => {
        ...
        if (selectedCat.type === 'device') {
            ...
            if (editId) updateDevice(editId, payload);
            ...
            if (editId) updateDeviceAccessory(editId, payload);
            else addDeviceAccessory(payload);
        }
    ...
    const openEdit = (item: any) => {
        setF({
            ...
            model: item.model || '', color: item.color || '',
    ...
    return (
        <div className="space-y-5 animate-fade-in" dir="rtl">
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div className="flex items-center gap-3">
                    <div className="flex h-11 w-11 items-center justify-center rounded-xl bg-orange-100 border border-orange-200 shadow-sm">
                        <Tv className="h-5 w-5 text-orange-600" />
                    </div>
                    <div>
                        <h1 className="text-2xl font-bold text-foreground">المنزليات وإكسسواراتها</h1>
            ...
            <div className="flex gap-2 w-full overflow-x-auto hide-scrollbar pb-2 pt-1 px-1 -mx-1">
                ...
                {categories.map(c => (
                    <button key={c.id} onClick={() => setActiveFilter(c.id)}
                        className={`shrink-0 rounded-xl px-4 py-2 text-sm font-semibold border transition-all flex items-center gap-2 ${activeFilter === c.id ? 'bg-orange-50 text-orange-700 border-orange-300 shadow-sm' : 'bg-card border-border text-muted-foreground hover:text-foreground hover:bg-muted/50'}`}>
                        {c.type === 'device' ? <Tv className="h-4 w-4 opacity-70" /> : <Headphones className="h-4 w-4 opacity-70" />}
                        {c.name}
                    </button>
                ))}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {filteredList.length === 0 ? (
                        <div className="col-span-4 py-20 text-center text-muted-foreground bg-card rounded-2xl border border-dashed border-border">
                            <Tv className="h-14 w-14 mx-auto mb-4 opacity-15" />
                        ...
                        <ProductCard key={item.id} item={item}
                            ...
                            onDelete={() => {
                                item._type === 'device' ? deleteDevice(item.id) : deleteDeviceAccessory(item.id);
                                        ...
                                        <tr key={item.id} className={`border-b border-border/40 hover:bg-muted/20 transition-colors ${i % 2 !== 0 ? 'bg-muted/10' : ''}`}>
                                            ...
                                            <td className="px-3 py-2 text-left">
                                                <div className="flex justify-end gap-1.5">
                                                    ...
                                                    <button onClick={() => { item._type === 'device' ? deleteDevice(item.id) : deleteDeviceAccessory(item.id); setTimeout(() => window.dispatchEvent(new Event('local-storage-sync')), 100); }} className="rounded-lg p-1.5 hover:bg-red-50 text-destructive transition-colors"><Trash2 className="h-3.5 w-3.5" /> </button>
                                                </div>
                                            </td>
                                        </tr>
                                    )
                        ...
                        <div className="p-4 space-y-4">
                            ...
                            {activeCategoryType === 'device' ? (
                                <div className="grid grid-cols-2 gap-3 bg-muted/20 p-3 rounded-xl border border-border/40">
                                    <div className="col-span-full mb-1"><span className="text-xs font-bold text-primary flex items-center gap-1"><Tv className="w-3.5 h-3.5" /> مواصفات الجهاز</span></div>
                    ...
                    <div className="w-full max-w-sm rounded-2xl bg-card border border-border shadow-2xl p-5 animate-scale-in">
                        ...
                        <div className="space-y-4">
                            ...
                            <div>
                                ...
                                <p className="text-[10px] text-muted-foreground/80 mt-1">يُشكل هذا الاختيار الحقول المطلوبة مستقبلاً للمنتجات ضمن هذا التصنيف.</p>
                            </div>
                        </div>
