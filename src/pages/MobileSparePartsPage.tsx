            ...
            <div className="w-full max-w-md rounded-2xl border border-border bg-card shadow-2xl animate-scale-in overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-muted/30">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-base font-bold">إدارة تصنيفات قطع غيار الموبايل</h3>
                        ...
                        <div key={idx}
                            ...
                            {editIdx === idx ? (
                                ...
                                <div className="flex-1 flex items-center gap-2">
                                    <span className="shrink-0 h-5 w-5 rounded-full flex items-center justify-center bg-cyan-100 text-cyan-600 dark:bg-cyan-500/10 dark:text-cyan-400">
                            ...
                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                {editIdx === idx ? (
                                    <>
                                        <button onClick={saveEdit} className="rounded-md p-1 hover:bg-emerald-50 dark:hover:bg-emerald-500/10 text-emerald-600">
                                            <Check className="h-3.5 w-3.5" />
                                        </button>
                                        <button onClick={() => setEditIdx(null)} className="rounded-md p-1 hover:bg-muted text-muted-foreground">
                                            <X className="h-3.5 w-3.5" />
                                        </button>
                                    </>
                                ) : (
                                    <>
                                        <button onClick={() => startEdit(idx)} className="rounded-md p-1 hover:bg-cyan-50 dark:hover:bg-cyan-500/10 text-cyan-600">
                                            <Pencil className="h-3.5 w-3.5" />
                                        </button>
                                        <button onClick={() => deleteCat(idx)} className="rounded-md p-1 hover:bg-red-50 dark:hover:bg-red-500/10 text-destructive">
...
export default function MobileSparePartsPage() {
    ...
    const [categories, setCategories] = useState<string[]>(() => loadCats('mobiles_spare_cats', ['شاشات', 'بطاريات', 'بوردة', 'فلاتات', 'كاميرات', 'سماعات داخلية', 'مايك']));
    const mobiles = useInventoryData(getMobileSpareParts, ['gx_mobiles_v2']);
    ...
    const unifiedProducts = useMemo(() => {
        const list: any[] = [];
        mobiles.forEach(m => {
            const cat = categories.find(c => c === m.category);
            list.push({
                _raw: m, _type: 'device',
                ...
                category: m.category, condition: m.condition || 'new', categoryName: cat,
                subcategory: (m as any).subcategory, model: m.model, color: m.color, supplier: m.supplier, serialNumber: m.serialNumber
            });
        });
        return list;
    }, [mobiles, categories]);
    ...
    // ── Handlers ──
    const refreshData = () => {
        setCategories(loadCats('mobiles_spare_cats', ['شاشات', 'بطاريات', 'بوردة', 'فلاتات', 'كاميرات', 'سماعات داخلية', 'مايك']));
        window.dispatchEvent(new Event('local-storage-sync'));
    };

    const handleSaveCats = (updated: string[]) => {
        saveCats('mobiles_spare_cats', updated);
    ...
    const handleFormSubmit = () => {
        ...
        if (editId) {
            ...
            updateMobileSparePart(editId, {
                name: f.name, barcode: u.barcode || f.barcode,
                category: f.category, subcategory: f.subcategory, model: f.model, condition: f.condition as any, quantity: f.quantity,
            ...
            validUnits.forEach(u => {
                addMobileSparePart({
                    name: f.name, barcode: u.barcode || `SM-${Date.now()}-${Math.random().toString(36).slice(2, 6)}`,
                    category: f.category, subcategory: f.subcategory, model: f.model, condition: f.condition as any,
        ...
        <div className="animate-fade-in" dir="rtl">
            ...
            <div className="flex gap-2 flex-wrap mb-4">
                ...
                <button onClick={() => navigate('/mobiles/spare-parts')}
                    className="flex items-center gap-2 rounded-xl bg-orange-600 px-5 py-2.5 text-sm font-bold text-white shadow-md ring-2 ring-orange-500 ring-offset-1">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-foreground">مخزون قطع غيار الموبايل</h1>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div><label className="mb-1 block text-xs font-semibold text-muted-foreground uppercase">الباركود</label><input value={f.barcode} onChange={e => setF(p => ({ ...p, barcode: e.target.value }))} placeholder="تلقائي" className={IC} /></div>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-muted/20 p-3 rounded-xl border border-border/40">
                                        <div className="col-span-full mb-1"><span className="text-xs font-bold text-primary flex items-center gap-1"><Wrench className="w-3.5 h-3.5" /> مواصفات القطعة</span></div>
                                        {/* نوع القطعة */}
                                        <div><label className="mb-1 block text-xs text-muted-foreground">النوع / التحديد</label><input value={f.subcategory} onChange={e => setF(p => ({ ...p, subcategory: e.target.value }))} placeholder="مثال: شاشة, بطارية..." className={IC} /></div>
                                        {/* الموديل */}
                                        <div><label className="mb-1 block text-xs text-muted-foreground">موديل الهاتف المطابق</label><input value={f.model} onChange={e => setF(p => ({ ...p, model: e.target.value }))} placeholder="الموديلات المتوافقة..." className={IC} /></div>
            ...
            <ExcelColumnMappingDialog
                ...
                onDataSave={(data) => {
                    data.forEach(row => {
                        const mobile: Omit<MobileSparePart, 'id' | 'createdAt' | 'updatedAt'> = {
                            name: row.name || '', barcode: row.barcode || '',
                            category: row.category || '', condition: row.condition as any || 'new',
                            quantity: Number(row.quantity) || 0, subcategory: row.subcategory || '', model: row.model || '',
