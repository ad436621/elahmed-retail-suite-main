...
import { exportToExcel, MOBILE_ACCESSORY_COLUMNS } from '@/services/excelService';
import {
            ...
            <div className="w-full max-w-md rounded-2xl border border-border bg-card shadow-2xl animate-scale-in overflow-hidden" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex items-center justify-between px-5 py-4 border-b border-border bg-muted/30">
                    <div className="flex items-center gap-2">
                        ...
                        <h3 className="text-base font-bold">إدارة تصنيفات إكسسوارات الموبايل</h3>
...
export default function MobileAccessoriesPage() {
    ...
    const [categories, setCategories] = useState<string[]>(() => loadCats('mobiles_acc_cats', ['سماعات سلك', 'سماعات بلوتوث', 'شواحن', 'كابلات', 'باور بنك', 'سكرينات', 'جرابات']));
    const mobiles = useInventoryData(getMobileAccessories, ['gx_mobiles_v2']);
    ...
    // ── Handlers ──
    const refreshData = () => {
        setCategories(loadCats('mobiles_acc_cats')); // Load categories again
        window.dispatchEvent(new Event('local-storage-sync'));
    };

    const handleSaveCats = (updated: string[]) => {
        saveCats('mobiles_acc_cats', updated);
        ...
        <div className="animate-fade-in" dir="rtl">
            ...
            <div className="flex gap-2 flex-wrap mb-4">
                ...
                <button onClick={() => navigate('/mobiles/accessories')}
                    className="flex items-center gap-2 rounded-xl bg-cyan-500 px-5 py-2.5 text-sm font-bold text-white shadow-md ring-2 ring-cyan-500 ring-offset-1">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-black text-foreground">مخزون إكسسوارات الموبايل</h1>
            ...
            <div className="flex flex-wrap items-center gap-2 mb-4">
                ...
                <button onClick={() => exportToExcel({ data: mobiles, columns: MOBILE_ACCESSORY_COLUMNS, fileName: 'اكسسورات_الموبايل' })}
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div><label className="mb-1 block text-xs font-semibold text-muted-foreground uppercase">الباركود</label><input value={f.barcode} onChange={e => setF(p => ({ ...p, barcode: e.target.value }))} placeholder="تلقائي" className={IC} /></div>
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-muted/20 p-3 rounded-xl border border-border/40">
                                        <div className="col-span-full mb-1"><span className="text-xs font-bold text-primary flex items-center gap-1"><Headphones className="w-3.5 h-3.5" /> مواصفات المنتج</span></div>
                                        {/* نوع الإكسسوار */}
                                        <div><label className="mb-1 block text-xs text-muted-foreground">النوع / التحديد</label><input value={f.subcategory} onChange={e => setF(p => ({ ...p, subcategory: e.target.value }))} placeholder="مثال: Type-C, شاشة جراب..." className={IC} /></div>
                                        {/* الموديل */}
                                        <div><label className="mb-1 block text-xs text-muted-foreground">موديل الهاتف المطابق</label><input value={f.model} onChange={e => setF(p => ({ ...p, model: e.target.value }))} placeholder="اختياري للإكسسوارات العامة..." className={IC} /></div>
            ...
            <ExcelColumnMappingDialog
                ...
                onDataSave={(data) => {
                    data.forEach(row => {
                        const mobile: Omit<MobileAccessory, 'id' | 'createdAt' | 'updatedAt'> = {
